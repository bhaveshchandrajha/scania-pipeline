...
## 6. Dockerfile (single image)

One stage: `python:3.11-slim-bookworm` with Node 20, Maven, JDK 17, and Git; `pip install -r requirements.txt`; copy repo; `CMD` runs `ui_global_context_server.py`. Angular and Maven builds run **inside the container** when you use the Pipeline UI (no separate Spring Boot image at repo root).
...
| Item | Description |
|------|-------------|
| `Dockerfile` | Pipeline image: Python UI + Node + Maven + Java |
| `requirements.txt` | Python deps (anthropic, etc.) |
| `docker-compose-pipeline.yml` | Ports 8003, 8081; optional env from `.env` |
...
3. Use `docker-compose-pipeline.yml` with volumes and env as needed.
