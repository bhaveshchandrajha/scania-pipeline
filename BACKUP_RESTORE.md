...
**Location:** `backup_checkpoint_YYYYMMDD_HHMMSS/` (inside project directory)
...
- Context packages
- AST files
- Excludes: `.git`, `__pycache__`, `*.class`, `target`, `.venv`, `backup_*`

**To restore from file backup:**
```bash
# Find backup directory
ls -ld backup_checkpoint_*

# Copy files back (be careful - this overwrites!)
cp -r backup_checkpoint_YYYYMMDD_HHMMSS/* .

# Or restore specific files
cp backup_checkpoint_YYYYMMDD_HHMMSS/migrate_with_claude.py .
```
...
# Verify key files
ls -lh backup_checkpoint_*/migrate_with_claude.py
ls -lh backup_checkpoint_*/context_index/*.json | wc -l
```
